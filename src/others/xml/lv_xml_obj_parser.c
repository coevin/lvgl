...
void lv_obj_xml_apply_attrs(lv_xml_parser_state_t * state, lv_obj_t * obj, const char ** attrs)
{
    for(int i = 0; attrs[i]; i += 2) {
        ...
        if(lv_streq("x", name)) lv_obj_set_x(obj, lv_xml_atoi(value));
        if(lv_streq("y", name)) lv_obj_set_y(obj, lv_xml_atoi(value));
        if(lv_streq("width", name)) lv_obj_set_width(obj, lv_xml_atoi(value));
        if(lv_streq("height", name)) lv_obj_set_height(obj, lv_xml_atoi(value));
